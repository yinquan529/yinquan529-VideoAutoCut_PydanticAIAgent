...
def _make_settings(**overrides) -> Settings:
    defaults = dict(
        hunyuan_api_key="test-key-123",
        hunyuan_base_url="https://api.test.example.com/v1",
        model_name="openai:hunyuan-turbos-latest",
        ffmpeg_path=Path("ffmpeg"),
        ffprobe_path=Path("ffprobe"),
        temp_frames_dir=Path("/tmp/frames"),
...
class TestCreateModel:
    ...
    def test_strips_openai_prefix(self):
        settings = _make_settings(model_name="openai:hunyuan-turbos-latest")
        model = create_model(settings)
        assert model.model_name == "hunyuan-turbos-latest"

    def test_no_prefix_kept_as_is(self):
        settings = _make_settings(model_name="some-custom-model")
        model = create_model(settings)
        assert model.model_name == "some-custom-model"

    def test_provider_uses_custom_base_url(self):
        settings = _make_settings(hunyuan_base_url="https://custom.api.com/v1")
        model = create_model(settings)
    ...
    def test_provider_uses_api_key(self):
        settings = _make_settings(hunyuan_api_key="my-secret-key")
        model = create_model(settings)
        client: AsyncOpenAI = model._provider._client
        assert client.api_key == "my-secret-key"

    def test_client_timeout_from_settings(self):
        settings = _make_settings(request_timeout_seconds=120)
        model = create_model(settings)
        client: AsyncOpenAI = model._provider._client
    ...
    def test_uses_get_settings_when_none(self, monkeypatch: pytest.MonkeyPatch):
        settings = _make_settings(model_name="openai:fallback-model")
        monkeypatch.setattr(
...
class TestCreateAgent:
    ...
    def test_retries_from_settings(self):
        settings = _make_settings(max_retries=4)
        agent = create_agent(settings)
...
class TestCreateStructuredAgent:
    ...
    def test_retries_from_settings(self):
        settings = _make_settings(max_retries=7)
        agent = create_structured_agent(_SampleOutput, settings)
